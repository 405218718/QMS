...
class zhujiemian(QMainWindow):
    def __init__(self, parent=None):
        ...
        # self.ui.ZhuCaiDan.tabBar().setTabButton(1, QTabBar.RightSide, None) # 第二项去掉关闭按钮
    ...
    def on_actFont_ranyuanchaxun_triggered(self, clicked):
        w = self.ui.ZhuCaiDan.indexOf(self.ui.renyuanxinxi)
        if w < 0:
            # self.ui.actFont_ranyuanchaxun.triggered.connect(self.add_tab_renyuanxinxi)
            DateTimeEdit(self.ui.ryxx_Q_riqi).resize(self.ui.ryxx_Q_riqi.width(), self.ui.ryxx_Q_riqi.height())
            DateTimeEdit(self.ui.ryxx_J_riqi).resize(self.ui.ryxx_J_riqi.width(), self.ui.ryxx_J_riqi.height())
            # # 新增选项卡（标题等同）
            self.tab['renyuanxinxi'] = [
                ...
                "离职日期",
                "备注"]
            self.add_tab(self.ui.renyuanxinxi,'人员信息',self.tab['renyuanxinxi'],self.ui.ryxx_tableWidget)
            self.ui.ryxx_bumen_text.setCurrentText("")
            self.ui.ryxx_gonghao__text.setText("")
            self.ui.ryxx_xingming.setText("")
    ...
    # 新增按钮
    @Slot()
    def on_ryxx_xinzeng_clicked(self):
    ...
    def on_actFont_xiangmuxinxi_triggered(self, clicked):
        ...
        DateTimeEdit(self.ui.xmxx_Q_riqi).resize(self.ui.xmxx_Q_riqi.width(), self.ui.xmxx_Q_riqi.height())
        DateTimeEdit(self.ui.xmxx_J_riqi).resize(self.ui.xmxx_J_riqi.width(), self.ui.xmxx_J_riqi.height())
        # # 新增选项卡
