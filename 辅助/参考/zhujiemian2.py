...
import pymysql
from PySide2.QtGui import QIcon, QFocusEvent, QValidator
from PySide2.QtWidgets import QApplication, QMessageBox, QWidget, QLabel, QMainWindow, QTabBar, \
    QTableWidgetItem, QAbstractItemView, QLineEdit, QAction, QHeaderView, QCalendarWidget, QDesktopWidget, QDateEdit
from 主界面 import Ui_MainWindow
from PySide2.QtCore import Slot, QDate, QDateTime, Qt, QEvent
...
class zhujiemian(QMainWindow):
    ...
    # QLineEdit日期显示
    def riqikuangxuanxiang(self,zujian:QLineEdit,show_action:QAction,rili:QCalendarWidget,def_qj,showDate):
        ...
        rili.setGeometry(zujian.x(),zujian.y()+zujian.height()+62, 280, 200)     #日历控件位置
        rili.setFocusPolicy(Qt.ClickFocus)
        rili.hide()  # 隐藏日期控件
        # date = self.rili.selectedDate()#获取选中日期，默认当前系统时间
        rili.clicked[QDate].connect(showDate)#clicked[参数]，即定义showDate是传入的参数类型设置


        # self.show_action.triggered.connect(self.showDate(date,zujian))
    def renyuanxinxi_QQ_showDate(self,date ):
        self.ui.ryxx_Q_riqi.setText(date.toString("yyyy/MM/dd"))
        # self.rili.close()#关闭日期控件
        self.renyuanxinxi_Q.hide()  # 隐藏日期控件
    def renyuanxinxi_JJ_showDate(self,date ):
        self.ui.ryxx_J_riqi.setText(date.toString("yyyy/MM/dd"))
        # self.rili.close()#关闭日期控件
        self.renyuanxinxi_J.hide()  # 隐藏日期控件
    def renyuanxinxi_QQ(self, event=None):
        if self.renyuanxinxi_Q.isHidden():
            self.renyuanxinxi_Q.show()  # 显示日期控件
            self.renyuanxinxi_Q.setFocus()   # 日期控件获取焦点

            # self.renyuanxinxi_Q.focusOutEvent(QFocusEvent * event)
        else:
            # self.rili.close()  # 关闭日期控件
            self.renyuanxinxi_Q.hide()  # 隐藏日期控件
    def renyuanxinxi_JJ(self):
        if self.renyuanxinxi_J.isHidden():
            self.renyuanxinxi_J.show()  # 显示日期控件
            self.renyuanxinxi_J.setFocus()  # 日期控件获取焦点
        else:
            # self.rili.close()  # 关闭日期控件
            self.renyuanxinxi_J.hide()  # 隐藏日期控件
    ...
    def mousePressEvent(self, event):   #鼠标左键按下
        ...
        # if not self.renyuanxinxi_Q.geometry().contains(self.underMouse()):
        # #     print(child.objectName())
        # # # # currentItem.clearFocus()
        if not self.renyuanxinxi_Q.hasFocus() or \
                event.pos() != self.renyuanxinxi_Q.visibleRegion():
            self.renyuanxinxi_Q.hide()
            print(child.objectName())
        if not self.renyuanxinxi_J.hasFocus():
            self.renyuanxinxi_J.hide()
        else:
            pass
        return False
...
# class rili():
#     def __init__(self, parent=None):
#         super(rili, self).__init__(parent=parent)
#         self.ui = Ui_MainWindow

class DateTimeEdit(QDateEdit):
